...
`;let Ql=!1;function n0(){if(Ql||typeof document>"u")return;const i=document.createElement("style");i.dataset.ch2="",i.textContent=t0,document.head.appendChild(i),Ql=!0}function vn(i){return i.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}let Qt=null,Yi=null;function r0(){if(typeof window>"u")return;const i=window.AudioContext??window.webkitAudioContext;i&&(Qt||(Qt=new i,Yi=Qt.createGain(),Yi.gain.value=.12,Yi.connect(Qt.destination)),Qt.state==="suspended"&&Qt.resume())}function Kl(i,e,t){if(r0(),!Qt||!Yi)return;const n=Qt.sampleRate,r=Math.max(2,Math.round(n/i)),s=Math.floor(n*e),o=Qt.createBuffer(1,s,n),a=o.getChannelData(0),l=new Float32Array(r);for(let d=0;d<r;d++)l[d]=Math.random()*2-1;let c=0;for(let d=0;d<s;d++){const h=l[c],f=l[(c+1)%r];l[c]=.996*.5*(h+f),a[d]=h*t,c=(c+1)%r}const u=Qt.createBufferSource();u.buffer=o,u.connect(Yi),u.start()}function i0(i){n0();const e=i.root.querySelector(".pin"),{copy:t}=i;function n(S,A){const Q=document.createElement(S);return Q.className=A,e.appendChild(Q),Q}const r=n("div","ch2-card ch2-title");r.innerHTML=`
  ...
  `;const p=g.querySelector(".ch2-scroll-body"),y=g.querySelector(".ch2-scroll-total"),v=g.querySelector(".ch2-scroll-count"),b=g.querySelector(".ch2-retreat"),x=new Map;for(const S of pa){const A=document.createElement("div");A.className="ch2-region",A.innerHTML=`<span>${S.name}</span><i class="ch2-region-bar"><b></b></i><em>0/0</em>`,p.appendChild(A),x.set(S.key,{bar:A.querySelector(".ch2-region-bar b"),num:A.querySelector("em")})}const w=n("div","ch2-card ch2-explore");w.innerHTML=`
    <h2>现在，把星空交给你</h2>
    <p>${vn(t.body[2]??"")}</p>
  `;const T=n("div","atlas-hint");T.textContent="拖拽环视 · 点击或凝视沉睡的星";let M=null,k=[];const R=new Map;Promise.all([fetch(On("data/poem.json")).then(S=>S.ok?S.json():null),fetch(On("data/stars.json")).then(S=>S.ok?S.json():null),fetch(On("data/asterisms.json")).then(S=>S.ok?S.json():null)]).then(([S,A,Q])=>{if(M=S,!A||!Q)return;const pe=new Map(A.stars.map(be=>[be.hip,be])),we=[];for(const be of Q.asterisms){const dt=be.stars.map(hr=>pe.get(hr)).filter(hr=>hr!==void 0),Sn=of(dt);if(!Sn)continue;const[jf,Qf,Kf]=Bt(Sn.ra,Sn.dec,Fo);let vo=3,fs=null;for(const hr of dt)vo=Math.max(vo,io(Sn,hr)),(fs===null||hr.mag<fs)&&(fs=hr.mag);const Zf=Math.max(6,Fo*Math.tan(vo*Math.PI/180)*1.35);we.push({name:be.name,region:S!=null&&S[be.name]?rf(S[be.name].from):null,ra:Sn.ra,dec:Sn.dec,x:jf,y:Qf,z:Kf,ring:Zf,mag:fs})}k=we,R.clear();for(const be of we)R.set(be.name,be);ut(),C===kn&&_t()}).catch(()=>{});let C=-1,P=0;const B=new Set(Yt());let m=B.size>0&&Li(B)===null,z=0,I=0,X=null,$=0;const F=new Wt;let O=!1,L=0,_=null,q=!1,K=[],D=null,W=null,U=null,ne=null,H=null,xe=0,Te=0;const at=new Set;let ze=0,Ee=!1;const V=new Wt;let ie=null,Ye="",tt=8,E=!1,Pe=null,lt=!1,gt=!1,me=!1,de=!1,Ie=!1,ct=!1,Fe=!1,_n=-2,Ae=!1;function Yt(){try{return cf(window.localStorage.getItem(ua))}catch{return[]}}function kt(){try{window.localStorage.setItem(ua,JSON.stringify([...B]))}catch{}}function Rt(){return k.length>0?k.length:i.sky.groupCount}function Un(S){const A=i.sky.groupCount;for(let Q=0;Q<A;Q++)i.sky.setGroupProgress(Q,S)}function Oe(){const S=i.sky.groupCount;for(let A=0;A<S;A++)i.sky.setGroupProgress(A,Ju);for(const A of B)i.sky.setGroupProgress(A,1)}function zn(S){lt!==S&&(lt=S,r.classList.toggle("on",S))}function Ot(S){gt!==S&&(gt=S,a.classList.toggle("on",S))}function yn(S){me!==S&&(me=S,w.classList.toggle("on",S))}function bn(S){de!==S&&(de=S,T.classList.toggle("on",S))}function Ht(S){_n!==S&&(_n=S,o.forEach((A,Q)=>A.classList.toggle("on",Q===S)))}function Xt(S){const A=S!==null;A&&(l.textContent=S),!(Ie===A&&!A)&&(Ie=A,l.classList.toggle("on",A))}function se(S){ct!==S&&(ct=S,c.classList.toggle("on",S),S||on(0))}function on(S){const A=da(S);u.style.borderColor=A>0?`rgba(201, 162, 39, ${.55+.45*A})`:"",u.style.transform=A>0?`scale(${1+.3*A})`:"",u.style.boxShadow=A>0?`0 0 ${8+10*A}px rgba(201, 162, 39, ${.3+.5*A})`:""}function mt(S){Fe!==S&&(Fe=S,g.classList.toggle("on",S))}function Lt(S,A){S.classList.remove(A),S.offsetWidth,S.classList.add(A)}function ut(){const S={ziwei:0,taiwei:0,tianshi:0,qinglong:0,xuanwu:0,baihu:0,zhuque:0},A={...S};for(const pe of k)pe.region&&(A[pe.region]+=1,B.has(pe.name)&&(S[pe.region]+=1));for(const pe of pa){const we=x.get(pe.key);if(!we)continue;const be=A[pe.key],dt=S[pe.key];we.bar.style.width=be>0?`${(dt/be*100).toFixed(1)}%`:"0%",we.num.textContent=`${dt}/${be}`}const Q=Rt();y.textContent=Q>0?`${B.size} / ${Q}`:`${B.size} / —`,v.textContent=`你已唤醒 ${B.size} 颗`}function In(){if(Pe)return Pe;const S=document.createElement("canvas");S.width=S.height=128;const A=S.getContext("2d");return A.strokeStyle="rgba(240, 205, 110, 0.95)",A.lineWidth=6,A.shadowColor="rgba(201, 162, 39, 0.9)",A.shadowBlur=14,A.beginPath(),A.arc(64,64,48,0,Math.PI*2),A.stroke(),Pe=new po(S),Pe}function Fn(S,A,Q,pe){const we=`${S.toFixed(1)},${A.toFixed(1)},${Q.toFixed(1)},${pe.toFixed(1)}`;if(ie&&Ye===we)return;ft();const be=new La({map:In(),transparent:!0,depthTest:!1,depthWrite:!1,opacity:.9}),dt=new Da(be);dt.position.set(S,A,Q),dt.scale.set(pe,pe,1),dt.renderOrder=998,i.sky.addSkyObject(dt),ie=dt,Ye=we,tt=pe}function ft(){ie&&(i.sky.removeSkyObject(ie),ie.material.dispose(),ie=null,Ye="")}function Nn(){ne!==null&&(clearTimeout(ne),ne=null)}function te(){for(const S of K)S.kill();K=[]}function Z(S,A){const Q={v:0},pe=An.to(Q,{v:1,duration:A,ease:"power1.out",onUpdate:()=>i.sky.setGroupProgress(S,Q.v),onComplete:()=>{K=K.filter(we=>we!==pe)}});K.push(pe)}function ue(){return ms+jp*Math.min(z,3)}const N={v:ms};function ee(S,A){D==null||D.kill(),D=An.to(N,{v:S,duration:A,ease:"power2.out",onUpdate:()=>i.sky.setBloom({strength:N.v}),onComplete:()=>{D=null}})}function j(){D==null||D.kill(),N.v=Vp,i.sky.setBloom({strength:N.v}),ee(ue(),.8)}const J={v:1};function Ue(){W==null||W.kill(),J.v=.5,i.sky.setTimeScale(.5),W=An.to(J,{v:1,delay:.4,duration:.6,ease:"power2.inOut",onUpdate:()=>i.sky.setTimeScale(J.v),onComplete:()=>{W=null}})}function fe(){W&&(W.kill(),W=null),J.v!==1&&(J.v=1,i.sky.setTimeScale(1))}function Ne(S){const A=M==null?void 0:M[S.name],Q=document.createElement("div");Q.className="ch2-poemfloat";const pe=A?lf(A.text,2):S.name,we=A?`《步天歌》 · ${A.from}`:"";Q.innerHTML=`<span class="ch2-poemfloat-text">${vn(pe)}</span>${we?`<span class="ch2-poemfloat-from">${vn(we)}</span>`:""}`;const be=qc([S.x,S.y,S.z],i.sky.camera,{width:window.innerWidth,height:window.innerHeight}),dt=be?be.x+40:window.innerWidth*.62,Sn=be?be.y:window.innerHeight*.42;Q.style.left=`${Math.min(Math.max(dt,110),window.innerWidth-110)}px`,Q.style.top=`${Math.min(Math.max(Sn,140),window.innerHeight-140)}px`,d.appendChild(Q),at.add(Q),Q.addEventListener("animationend",()=>{at.delete(Q),Q.remove()})}function nt(){at.forEach(S=>S.remove()),at.clear()}function Le(S){const A=R.get(S);if(!A||C===Oi||(Fn(A.x,A.y,A.z,A.ring),!ie))return;U==null||U.kill(),E=!0;const Q={o:0};ie.material.opacity=0,U=An.to(Q,{o:.8,duration:.9,ease:"sine.inOut",yoyo:!0,repeat:1,onUpdate:()=>{ie&&(ie.material.opacity=Q.o)},onComplete:()=>{U=null,E=!1,ft()}})}function Ce(){_||(_=qp()),q||(i.sky.addSkyObject(_.group),q=!0)}function ge(){return C!==kn?null:Li(B)}function _t(){if(C!==kn)return;const S=Li(B);if(S){const A=tl.indexOf(S);Xt(`第${e0[A]??A+1}站 · 「${S}」——跟着星使：点它，或凝视它`);const Q=R.get(S);Q&&(Ce(),_.setVisible(!0),_.flyTo({x:Q.x,y:Q.y,z:Q.z}));return}Xt(null),m||(m=!0,Re())}function Re(){Lt(h,"on"),_&&(_.pulse(1),Nn(),ne=setTimeout(()=>{ne=null,_==null||_.setVisible(!1)},Kp))}function yt(S){if(!ha(S,ge(),B))return;const A=R.get(S);B.add(S),kt(),i.sky.hideDetailCard(),Ue(),Z(S,1.1),A&&(i.sky.spawnBurst({x:A.x,y:A.y,z:A.z},{count:90}),Ne(A),Kl(af(A.mag??4.5),.9,.85)),j(),_&&C===kn&&_.pulse(1),I=0,X=null,$=0,ut(),an(),_t()}function an(){const S=Gs(B.size,Rt());if(!(S<=z)){if(z=S,z>=4){Ke();return}ee(ue(),1.5),i.sky.spawnMeteors(z)}}function Ke(){ee(ue(),1.5),i.sky.spawnMeteors(8),Lt(f,"on"),g.classList.add("done"),Kl(523.25,1.4,.8)}function Ze(){B.clear(),kt(),m=!1,z=0,Nn(),h.classList.remove("on"),f.classList.remove("on"),g.classList.remove("done"),te(),ee(ms,.9),Oe(),I=0,X=null,$=0,L=0,ut(),C===kn&&_t()}b.addEventListener("click",Ze);function rt(S){if($=0,!S||C!==kn&&C!==fa)return;const A=S.info.name;ha(A,ge(),B)&&yt(A)}const ln=new re;function fr(){return i.sky.camera.getWorldDirection(ln),{ra:Math.atan2(ln.z,ln.x)*180/Math.PI,dec:Math.asin(Math.min(1,Math.max(-1,ln.y)))*180/Math.PI}}function Ve(){return C===kn||C===fa}function Xe(S){if(!Ve()||k.length===0||S<=0)return;const A=fr(),pe=ge()??ga(k,B,A),we=pe?R.get(pe):null,be=we?io(A,we):1/0;se(we!==null&&be<Up),we&&be<ef?(X!==we.name&&(X=we.name,I=0),I+=S,on(I/Bs),I>=Bs&&(on(0),yt(we.name))):(I>0||X!==null)&&(I=0,X=null,on(0))}function cn(S){if(!Ve()||ge()!==null||k.length===0){$=0;return}const A=i.sky.camera.quaternion;if(!O){O=!0,F.copy(A);return}if(F.angleTo(A)>4e-4){$=0,F.copy(A);return}if($+=S,$>=tf){$=0;const Q=ga(k,B,fr());Q&&Le(Q)}}function mi(S){if(!Ve()||z<1)return;const A=Qp[Math.min(z,4)-1];if(L<=0){L=S+A;return}S>=L&&(L=S+A,i.sky.spawnMeteors(1))}function _i(S){xe=requestAnimationFrame(_i);const A=Te>0?Math.min((S-Te)/1e3,.1):0;if(Te=S,_&&q&&(_.update(A),C===kn&&ge()!==null)){const Q=.35+.2*Math.sin(S*.003);_.pulse(Math.min(1,Q+I/Bs*.5))}if(Xe(A),cn(A),mi(S),ie&&!E){const Q=tt*(1+.13*Math.sin(S*.0024));ie.scale.set(Q,Q,1),ie.material.opacity=.7+.3*Math.sin(S*.0024+1)}}function dr(S){S===Oi?(i.sky.setPickingEnabled(!1),i.sky.setLabelsEnabled(!0),i.sky.setHoverTipEnabled(!0),Un(0),Ae=!1,Xt(null),se(!1),mt(!1),yn(!1),bn(!1),_==null||_.setVisible(!1),ft()):S===kn?(i.sky.setPickingEnabled(!0),i.sky.setLabelsEnabled(!1),i.sky.setHoverTipEnabled(!0),zn(!1),Ht(-1),Ot(!1),yn(!1),bn(!1),ft(),Oe(),z=Gs(B.size,Rt()),i.sky.setBloom({strength:ue()}),g.classList.toggle("done",z>=4),m=Li(B)===null,ut(),mt(!0),_t()):(i.sky.setPickingEnabled(!0),i.sky.setLabelsEnabled(!0),i.sky.setHoverTipEnabled(!0),zn(!1),Ht(-1),Ot(!1),Xt(null),_==null||_.setVisible(!1),ft(),Oe(),z=Gs(B.size,Rt()),i.sky.setBloom({strength:ue()}),g.classList.toggle("done",z>=4),ut(),mt(!0),yn(!0),bn(!0))}function un(S){const A=ma(S);(A.finale>0||Ae)&&(Un(A.finale),Ae=A.finale>0),A.lines.forEach((pe,we)=>{const be=jl[we];if(!be)return;const dt=Math.max(A.finale,we===A.active?pe:pe*.15);for(const Sn of be.groups)i.sky.setGroupProgress(Sn,dt)}),zn(S<Sr);const Q=S>=Sr&&S<ii?A.active:-1;if(Ht(Q),Q>=0){const pe=Zp[Q];if(pe){const[we,be,dt]=Bt(pe.ra,pe.dec,Fo);Fn(we,be,dt,pe.ring)}}else ft();Ot(S>=ii)}function Vn(S){P=S;const A=nf(S);A!==C&&(C=A,dr(A)),C===Oi&&un(S)}function Gr(S){const A=C===Oi&&P>=Sr&&P<ii?ma(P).active:-1,Q=A>=0?.85:0;if(ze+=(Q-ze)*(1-Math.exp(-3*S)),ze<.01){Ee&&(Ee=!1,i.sky.setGazeBlend(0));return}const pe=Jp[Math.max(A,0)];Ee?V.slerp(pe,1-Math.exp(-2.5*S)):(Ee=!0,V.copy(pe)),i.sky.setGazeBlend(ze,V)}return{enter(){i.root.classList.add("inview"),H==null||H(),H=i.sky.onPick(rt),xe&&cancelAnimationFrame(xe),Te=0,xe=requestAnimationFrame(_i),Vn(P)},update(S){Vn(S)},frame(S){Gr(S)},exit(){i.root.classList.remove("inview"),cancelAnimationFrame(xe),xe=0,Te=0,H==null||H(),H=null,Nn(),te(),U==null||U.kill(),U=null,E=!1,fe(),D&&(D.kill(),D=null),i.sky.setBloom({strength:ms}),ft(),Pe==null||Pe.dispose(),Pe=null,_&&q&&(_.setVisible(!1),i.sky.removeSkyObject(_.group),q=!1),nt(),h.classList.remove("on"),f.classList.remove("on"),I=0,X=null,$=0,O=!1,L=0,ze=0,Ee=!1,i.sky.setGazeBlend(0),i.sky.setLabelsEnabled(!0),i.sky.setHoverTipEnabled(!0),i.sky.setPickingEnabled(!1);for(const S of B)i.sky.setGroupProgress(S,1);zn(!1),Ht(-1),Ot(!1),Xt(null),se(!1),mt(!1),yn(!1),bn(!1),C=-1,Qt==null||Qt.suspend()}}}const s0=Object.freeze(Object.defineProperty({__proto__:null,CH2_GAZE_ANGLE_DEG:ef,CH2_GAZE_HOLD_S:Bs,CH2_GUIDE_STATIONS:tl,CH2_IDLE_PULSE_S:tf,CH2_REGIONS:pa,CH2_SEG1_END:el,CH2_SEG1_LINE_COUNT:$s,CH2_SEG2_END:Zu,CH2_SLEEP_DIM:Ju,CH2_STORAGE_KEY:ua,CH2_UNLOCKS:sf,ch2AngularDistanceDeg:io,ch2CanAwaken:ha,ch2Centroid:of,ch2GuideTarget:Li,ch2NearestSleeping:ga,ch2ParseAwakened:cf,ch2PluckFreq:af,ch2PoemExcerpt:lf,ch2RegionOf:rf,ch2Seg1LineStates:ma,ch2SegmentOf:nf,ch2UnlockTier:Gs,createChapter:i0},Symbol.toStringTag,{value:"Module"})),uf=Math.PI/180,o0=34.7,ff=8,Jr=355,_a=["冬至","小寒","大寒","立春","雨水","惊蛰","春分","清明","谷雨","立夏","小满","芒种","夏至","小暑","大暑","立秋","处暑","白露","秋分","寒露","霜降","立冬","小雪","大雪"];function a0(i){return-23.44*Math.cos(2*Math.PI*(i+10)/365.25)}function ya(i){return 90-Math.abs(o0-a0(i))}function Zl(i){return ff/Math.tan(ya(i)*uf)}function l0(i){let e=0,t=999,n=0;for(let r=0;r<_a.length;r++){const s=(Jr+r*15.22)%365;let o=i-s;o>182.5?o-=365:o<-182.5&&(o+=365),Math.abs(o)<t&&(t=Math.abs(o),e=r,n=o)}return{name:_a[e],index:e,day:(Jr+e*15.22)%365,offset:Math.round(n)}}function c0(i){const e=[31,28,31,30,31,30,31,31,30,31,30,31];let t=Math.min(Math.max(Math.round(i),1),365),n=0;for(;n<11&&t>e[n];)t-=e[n],n++;return{month:n+1,day:t}}const _s=["零","一","二","三","四","五","六","七","八","九"];function No(i){if(i<10)return _s[i];if(i<20)return"十"+(i%10?_s[i%10]:"");const e=Math.floor(i/10);return _s[e]+"十"+(i%10?_s[i%10]:"")}function Hr(i){return i-Math.floor(i)}function u0(i,e,t,n,r,s){i.beginPath(),i.moveTo(e+s,t),i.arcTo(e+n,t,e+n,t+r,s),i.arcTo(e+n,t+r,e,t+r,s),i.arcTo(e,t+r,e,t,s),i.arcTo(e,t,e+n,t,s),i.closePath()}function f0(){const i=document.createElement("canvas");i.width=64,i.height=64;const e=i.getContext("2d");if(e){const t=e.createRadialGradient(32,32,2,32,32,32);t.addColorStop(0,"rgba(252, 225, 182, 0.9)"),t.addColorStop(.3,"rgba(252, 225, 182, 0.25)"),t.addColorStop(1,"rgba(252, 225, 182, 0)"),e.fillStyle=t,e.fillRect(0,0,64,64)}return i}const Xr=8,_r=15,d0=`
.gw {
...
`;let Jl=!1;function h0(){if(Jl||typeof document>"u")return;const i=document.createElement("style");i.dataset.gnomonWidget="",i.textContent=d0,document.head.appendChild(i),Jl=!0}function p0(i={}){h0();const e=document.createElement("div");e.className="gw",e.setAttribute("role","group"),e.setAttribute("aria-label","圭表测影：拖动滑杆查看一年中正午日影变化");const t=document.createElement("canvas");t.className="gw-canvas",e.appendChild(t);const n=document.createElement("div");n.className="gw-readout",n.innerHTML=`
...
`;let ec=!1;function m0(){if(ec||typeof document>"u")return;const i=document.createElement("style");i.dataset.gnomonLayout="",i.textContent=g0,document.head.appendChild(i),ec=!0}function vi(i){return i.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function tc(i){return i/365*Math.PI*2}function _0(i){const e=i.root.querySelector(".pin"),{copy:t}=i,n=document.createElement("div");n.className="gnomon-layout";const r=document.createElement("div");r.className="chapter-panel",r.innerHTML=`
...
`;let sc=!1;function E0(){if(sc||typeof document>"u")return;const i=document.createElement("style");i.dataset.ch4="",i.textContent=T0,document.head.appendChild(i),sc=!0}function wi(i){return i.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function C0(i){E0();const e=i.root.querySelector(".pin"),{copy:t}=i,n=document.createElement("div");n.className="ch4-card ch4-opening",n.innerHTML=`
    <p class="eyebrow">${wi(t.eyebrow)}</p>
    <div class="chapter-head">
      <h2>${wi(t.title)}</h2>
      ${t.seal?`<div class="seal">${wi(t.seal)}</div>`:""}
    </div>
    <p class="ch4-opening-tag">${w0}</p>
  ...
  `,i.root.querySelector(".pin").appendChild(t);let n=null,r=0;const s={v:0};function o(a){if(!n)return;n.setAssembly(Math.min(a/Go,1));const l=Math.max(0,(a-Go)/(1-Go));n.setSpin(l*ig)}return{enter(){i.root.classList.add("inview"),n||(n=rg(),i.sky.addSkyObject(n.group,{rotateWithSky:!1}),o(r)),An.to(s,{v:1,duration:sg,ease:"power2.out",overwrite:!0,onUpdate:()=>n==null?void 0:n.setFade(s.v)})},update(a){r=a,o(a)},exit(){i.root.classList.remove("inview"),An.killTweensOf(s),s.v=0,n&&(i.sky.removeSkyObject(n.group),n.dispose(),n=null)}}}const ag=Object.freeze(Object.defineProperty({__proto__:null,createChapter:og},Symbol.toStringTag,{value:"Module"})),kr=-1e4,oo=14e3,wa=oo-kr,lg=[{name:"帝星",years:-1e3,note:"−1000"},{name:"勾陈一",years:0,note:"今"},{name:"织女一",years:13700,note:"+13700"}],cg=[{years:kr,text:"−10000",cls:"ch6-endlab--start"},{years:0,text:"0",cls:""},{years:oo,text:"+14000",cls:"ch6-endlab--end"}],ug=2e3,fg=1.5,dg=.07,hg=`
...
`;let mc=!1;function pg(){if(mc||typeof document>"u")return;const i=document.createElement("style");i.dataset.ch6="",i.textContent=hg,document.head.appendChild(i),mc=!0}function Ti(i){return i.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function ws(i){return(i-kr)/wa*100}function gg(i){const e=2e3+i;return e<=0?{era:"公元前",num:1-e}:{era:e<3e3?"公元":"公元后",num:e}}function mg(i){pg();const e=i.root.querySelector(".pin"),t=document.createElement("div");t.className="chapter-panel ch6-panel",t.innerHTML=`
  ...
  `,e.appendChild(t);const n=document.createElement("div");n.className="ch6-time";const r=[];for(let y=kr;y<=oo;y+=ug){const v=y===kr||y===0||y===oo;r.push(`<div class="ch6-tick${v?" ch6-tick--major":""}" style="left:${ws(y).toFixed(3)}%"></div>`)}const s=cg.map(y=>`<div class="ch6-endlab ${y.cls}" style="left:${ws(y.years).toFixed(3)}%">${y.text}</div>`),o=lg.map(y=>`
  ...
  `,e.appendChild(n);const a=n.querySelector(".ch6-era"),l=n.querySelector(".ch6-num"),c=n.querySelector(".ch6-pointer");let u=null;function d(){const y=new jc(fg,dg,12,96),v=new md({color:13214247}),b=new js(y,v);return b.rotation.x=Math.PI/2,b.position.set(0,1.01*ye,0),b}let h=0,f=Number.NaN,g=Number.NaN;function p(y){i.sky.setSkyRotation(0,y);const v=Math.round(y);if(v!==f){f=v;const{era:x,num:w}=gg(v);a.textContent=x,l.textContent=String(w)}const b=Math.round(ws(y)*100)/100;b!==g&&(g=b,c.style.left=`${b}%`)}return{enter(){i.root.classList.add("inview"),u=d(),i.sky.addSkyObject(u,{rotateWithSky:!1}),p(kr+h*wa)},update(y){h=y,p(kr+y*wa)},exit(){i.root.classList.remove("inview"),i.sky.setSkyRotation(0,0),u&&(i.sky.removeSkyObject(u),u.geometry.dispose(),u.material.dispose(),u=null)}}}const _g=Object.freeze(Object.defineProperty({__proto__:null,createChapter:mg},Symbol.toStringTag,{value:"Module"})),yg=100,bg=9414856;async function _c(i){const e=await fetch(i);if(!e.ok)throw new Error(`${i} → HTTP ${e.status}`);return e.json()}async function vg(){const[i,e]=await Promise.all([_c(On("data/western.json")),_c(On("data/stars.json"))]),t=new Map;for(const l of e.stars)t.set(l.hip,Bt(l.ra,l.dec,yg));const n=[];for(const l of i.constellations)for(const[c,u]of l.lines){const d=t.get(c),h=t.get(u);!d||!h||n.push(d[0],d[1],d[2],h[0],h[1],h[2])}const r=new Zi;r.setAttribute("position",new nr(new Float32Array(n),3));const s=new Vc({color:bg,transparent:!0,opacity:0,depthWrite:!1,blending:zr}),o=new _d(r,s);o.name="western-lines",o.frustumCulled=!1;const a=new wn;return a.name="western",a.add(o),a.visible=!1,{group:a,setOpacity(l){const c=Me.clamp(l,0,1);s.opacity=c,a.visible=c>.001},dispose(){r.dispose(),s.dispose()}}}const yc=.6,xg=`
.ch7-panel {
  position: absolute;
  left: 6vw;
  top: 10vh;
...
`;let bc=!1;function wg(){if(bc||typeof document>"u")return;const i=document.createElement("style");i.dataset.ch7="",i.textContent=xg,document.head.appendChild(i),bc=!0}function Ei(i){return i.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Sg(i){return i=Me.clamp(i,0,1),i*i*(3-2*i)}function kg(i){wg();const e=i.root.querySelector(".pin"),{copy:t}=i,n=document.createElement("div");n.className="ch7-panel",n.innerHTML=`
...
`;let vc=!1;function Cg(){if(vc||typeof document>"u")return;const i=document.createElement("style");i.dataset.ch8="",i.textContent=Eg,document.head.appendChild(i),vc=!0}function En(i){return i.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Mg(i){return i<0?0:i>1?1:i}function Pg(i){return i.split(/(https?:\/\/\S+)/g).map(e=>/^https?:\/\//.test(e)?`<a href="${En(e)}" target="_blank" rel="noopener">${En(e)}</a>`:En(e)).join("")}function xc(i,e,t){const n=Mg((i-e)/(t-e));return n*n*(3-2*n)}function Ag(i){Cg();const e=i.root.querySelector(".pin"),{copy:t}=i,n=document.createElement("div");n.className="ch8-wrap",n.innerHTML=`
    <div class="ch8-panel">
...
`;function um(i){const e=new re(...Bt(Yc.ra,Yc.dec)).normalize(),t=new re(...Bt(Hc.ra,Hc.dec)),n=t.addScaledVector(e,-t.dot(e)).normalize(),r=new re().crossVectors(e,n).normalize(),s=new yd(i*tm,96,64),o=new Wc({vertexShader:lm,fragmentShader:cm,uniforms:{uPole:{value:e},uE0:{value:n},uE1:{value:r},uPeakAlpha:{value:nm},uWidth:{value:rm},uCenterSigma:{value:im},uDust:{value:sm},uColorBand:{value:om},uColorCore:{value:am}},transparent:!0,depthWrite:!1,blending:zr,side:bd}),a=new js(s,o);a.name="milkyway-shell";const l=new wn;return l.name="milkyway",l.add(a),{group:l,dispose(){s.dispose(),o.dispose()}}}function Vf(){document.fullscreenEnabled&&(document.fullscreenElement?Promise.resolve(document.exitFullscreen()).catch(()=>{}):Promise.resolve(document.documentElement.requestFullscreen()).catch(()=>{}))}const fm=`
.app-pager {
...
`;function dm({sections:i,names:e}){const t=document.createElement("style");t.textContent=fm,document.head.appendChild(t);const n=document.createElement("div");n.className="app-pager";const r=document.createElement("button");r.className="app-pager-btn",r.type="button",r.setAttribute("aria-label","上一章"),r.textContent="‹";const s=document.createElement("span");s.className="app-pager-idx";const o=document.createElement("button");if(o.className="app-pager-btn",o.type="button",o.setAttribute("aria-label","下一章"),o.textContent="›",document.fullscreenEnabled){const h=document.createElement("button");h.className="app-pager-btn",h.type="button";const f=()=>{const g=!!document.fullscreenElement;h.textContent=g?"✕":"⛶",h.setAttribute("aria-label",g?"退出全屏（F）":"进入全屏（F）")};h.addEventListener("click",Vf),document.addEventListener("fullscreenchange",f),f(),n.append(r,s,o,h)}else n.append(r,s,o);document.body.appendChild(n);const a=i.length-1;let l=0;function c(){const h=window.innerHeight,f=[];for(const g of i){const p=g.offsetTop,y=Math.max(g.offsetHeight-h,0),v=Math.round(y/h);for(let b=0;b<=v;b++)f.push(p+Math.min(b*h,y))}return f.sort((g,p)=>g-p)}function u(){s.textContent=e[l]?`${e[l]} · ${l+1}/${i.length}`:`${l+1}/${i.length}`;const h=document.documentElement.scrollHeight-window.innerHeight;r.disabled=window.scrollY<=2,o.disabled=window.scrollY>=h-2}function d(h){const f=c(),g=window.scrollY,p=2,y=h>0?f.find(v=>v>g+p)??f[f.length-1]:[...f].reverse().find(v=>v<g-p)??0;y!==void 0&&window.scrollTo({top:y,behavior:"smooth"})}return r.addEventListener("click",()=>d(-1)),o.addEventListener("click",()=>d(1)),window.addEventListener("scroll",u,{passive:!0}),u(),{setCurrent(h){const f=Math.min(Math.max(Math.round(h),0),a);f!==l&&(l=f,u())}}}const hm=3.5;function pm(){try{const i=document.createElement("canvas");return!!(i.getContext("webgl2")||i.getContext("webgl"))}catch{return!1}}function Xc(i){var n,r,s;const e=document.getElementById("fallback");e&&(e.hidden=!1);const t=document.getElementById("fallback-diag");t&&(t.textContent=`诊断信息：${i}`),(n=document.getElementById("chapters"))==null||n.setAttribute("hidden",""),(r=document.getElementById("sky-canvas"))==null||r.setAttribute("hidden",""),(s=document.getElementById("loading"))==null||s.remove()}async function gm(){const i=document.getElementById("sky-canvas");if(!i)throw new Error("缺少 #sky-canvas");const e=new xa(i);em(i);const t=document.getElementById("loading");try{await e.init()}catch(u){console.error(u),t&&(t.textContent="星空数据加载失败，请检查开发服务器");return}t==null||t.remove(),e.addSkyObject(um(ye).group),qd();const n=new Qs(Qd),r=[1,2,3,4,5,6,7,8].map(u=>document.getElementById(`ch${u}`)),s=["序","星野","授时","天人","天球","岁差","对话","尾声"],o=dm({sections:r,names:s});window.addEventListener("keydown",u=>{if(u.key!=="f"&&u.key!=="F"||u.ctrlKey||u.metaKey||u.altKey)return;const d=u.target;d&&(d.tagName==="INPUT"||d.tagName==="TEXTAREA"||d.isContentEditable)||Vf()});let a=0,l=0;const{chapters:c}=Zg(e,u=>{a=u,o.setCurrent(Math.min(Math.floor(u),s.length-1))});e.start(u=>{var h,f;l+=(a-l)*(1-Math.exp(-u*hm)),e.applyCameraState(n.sampleGlobal(l));const d=Math.min(Math.max(Math.floor(l),0),c.length-1);(f=(h=c[d])==null?void 0:h.frame)==null||f.call(h,u)})}pm()?gm().catch(i=>{console.error(i),Xc(i instanceof Error?i.message:String(i))}):Xc("当前浏览器环境无法创建 WebGL 上下文（webgl2 / webgl 均不可用）");
